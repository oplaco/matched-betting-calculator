...
class BackLayAccumulatedCalculator(CalculatorBase):
    ...
    def create_equations(self):
        ...
        for i in range(0, n):
            ...
            bb = group.back_bet
            lb = group.lay_bet
            eq = lay_stakes[i] * (1 - lb.fee / 100) - sum([lay_stakes[j] * (self.back_ley_groups[j].lay_bet.odds - 1) for j in range(i)]) - self.combo_stake - balance
            equations.append(eq)
        ...
        final_eq = self.combo_stake * (total_odds*(1-self.combo_fee/100) - 1) - sum([lay_stakes[i] * (self.back_ley_groups[i].lay_bet.odds - 1) for i in range(n)]) - balance
